...
from utils import RpcReportClient, RpcEmailClient, RpcInfoClient, ProjectEnv
...
async def get_crypto_report(
    currency: str = Query(default="usd", description="Divisa (usd, eur, etc.)")
):
    """
...
async def get_bar_graph(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al generar el gráfico: {str(e)}"
        )
...
async def subscribe(subscription: SubscriptionRequest):
    ...
    try:
        ...
        response = rpc_email.subscribe_email(
            subscription.email, subscription.hour, subscription.minute
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al procesar la suscripción: {str(e)}"
        )
...
async def get_exchanges():
    """
    Devolver lista de cambio de moneda en base al dolar
    """
    try:
        log.info(f"Solicitando cambio de moneda")
        data = rpc_info.get_exchanges()
        response = [ExchangeRate.from_proto(d) for d in data.rates]
        return response
    except Exception as e:
        log.error(f"Error obteniendo cambio de moneda: {e}")
        raise HTTPException(
            status_code=500, detail=f"Error al obtener el cambio de moneda: {str(e)}"
        )
...
if __name__ == "__main__":
    ...
    uvicorn.run("main:app", host=ProjectEnv.BACKEND_HOST, port=port, reload=True)
